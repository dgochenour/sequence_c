...
rtiddsgen -language C -example <arch> example.idl
...
void MyTypeListener_on_requested_incompatible_qos(
    void* listener_data,
    DDS_DataReader* reader,
...
void MyTypeListener_on_sample_rejected(
    void* listener_data,
    DDS_DataReader* reader,
...
void MyTypeListener_on_liveliness_changed(
    void* listener_data,
    DDS_DataReader* reader,
...
void MyTypeListener_on_sample_lost(
    void* listener_data,
    DDS_DataReader* reader,
...
void MyTypeListener_on_subscription_matched(
    void* listener_data,
    DDS_DataReader* reader,
...
void MyTypeListener_on_data_available(
    void* listener_data,
    DDS_DataReader* reader)
{
    MyTypeDataReader *MyType_reader = NULL;
    struct MyTypeSeq data_seq = DDS_SEQUENCE_INITIALIZER;
    struct DDS_SampleInfoSeq info_seq = DDS_SEQUENCE_INITIALIZER;
    DDS_ReturnCode_t retcode;
    int i;

    MyType_reader = MyTypeDataReader_narrow(reader);
    if (MyType_reader == NULL) {
        fprintf(stderr, "DataReader narrow error\n");
        return;
    }

    retcode = MyTypeDataReader_take(
        MyType_reader,
    ...
    retcode = MyTypeDataReader_return_loan(
        MyType_reader,
        &data_seq, &info_seq);
...
int subscriber_main(int domainId, int sample_count)
{
    ...
    type_name = MyTypeTypeSupport_get_type_name();
    retcode = MyTypeTypeSupport_register_type(participant, type_name);
    if (retcode != DDS_RETCODE_OK) {
    ...
    MyTypeListener_on_requested_deadline_missed;
    reader_listener.on_requested_incompatible_qos =
    MyTypeListener_on_requested_incompatible_qos;
    reader_listener.on_sample_rejected =
    MyTypeListener_on_sample_rejected;
    reader_listener.on_liveliness_changed =
    MyTypeListener_on_liveliness_changed;
    reader_listener.on_sample_lost =
    MyTypeListener_on_sample_lost;
    reader_listener.on_subscription_matched =
    MyTypeListener_on_subscription_matched;
    reader_listener.on_data_available =
    MyTypeListener_on_data_available;
    ...
    for (count=0; (sample_count == 0) || (count < sample_count); ++count) {
        printf("MyType subscriber sleeping for %d sec...\n",
        poll_period.sec);
